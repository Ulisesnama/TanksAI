...
public class AdvancedFSM : FSM 
{
    ...
    public AdvancedFSM()
    {
		this.fsmStates = new List<FSMState>();
    }
    ...
    public void AddFSMState(FSMState fsmState)
    {
		...
		if (this.fsmStates.Count == 0)
        {
			this.fsmStates.Add(fsmState);
			this.currentState = fsmState;
			this.currentStateID = fsmState.ID;

            return;
        }

        // Add the state to the List if it´s not inside it
		foreach (FSMState state in this.fsmStates)
        {
    ...
    public void PerformTransition(Transition trans)
    {
		...
		FSMStateID id = this.currentState.GetOutputState(trans);
